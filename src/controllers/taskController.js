...
import { scheduleTasks } from "../utils/helper.js";
...
export const updateTask = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const logId = uuid();
    const completedOn = new Date().toISOString().slice(0, 19).replace("T", " ");
...
export const addTask = catchAsync(async (req, res, next) => {
  ...
  const [user] = await pool.query(`SELECT * FROM user_base WHERE userId = ?`, [
    relatedUserId,
  ]);
