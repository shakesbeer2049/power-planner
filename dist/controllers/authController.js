...
export const register = catchAsync(async (req, res) => {
    ...
    const userId = uuid();
    // create user
    const queryResponse = await pool.query("INSERT INTO user_base (userId, username, email, password) VALUES (?, ?, ?, ?)", [userId, username, email, hashedPassword]);
    console.log(queryResponse, "new user");
    const [userResult] = await pool.query("SELECT userId FROM user_base WHERE email = ?", [email]);
    const newUser = {
...
export const login = catchAsync(async (req, res, next) => {
    ...
    const queryResponse = await pool.query("SELECT * FROM user_base WHERE email = ?", [email]);
...
export const protect = catchAsync(async (req, res, next) => {
    ...
    const queryResponse = await pool.query(`SELECT * FROM user_base WHERE userId = ?`, [id]);
...
export const forgotPassword = catchAsync(async (req, res, next) => {
    const queryResponse = await pool.query(`SELECT * FROM user_base WHERE email = ?`, [req.body.email]);
