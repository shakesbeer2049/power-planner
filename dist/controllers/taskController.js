...
export const getAllTasks = catchAsync(async (req, res, next) => {
    ...
    WHERE TAB.relatedUserId = ?;`, [req.user.userId]);
    res
        .status(200)
        .json({ status: "success", data: { tasks }, count: tasks.length });
});
export const getTasksToday = catchAsync(async (req, res, next) => {
    const [tasks] = await pool.query(`SELECT 
    TAB.taskId,
    TAB.taskName,
    TAB.createdOn,
    ...
    AND TAS.scheduledOn >= CURDATE();`, [req.user.userId]);
    console.log("tasks", tasks);
    res
        .status(200)
        .json({ status: "success", data: { tasks }, count: tasks.length });
});
export const getTasksForThisWeek = catchAsync(async (req, res, next) => {
    const [tasks] = await pool.query(`SELECT 
    TAB.taskId,
    TAB.taskName,
    TAB.createdOn,
...
ORDER BY TAS.scheduledOn, TAB.taskId;`, [req.user.userId]);
    console.log("tasks", tasks);
    res
        .status(200)
        .json({ status: "success", data: tasks || [], count: tasks.length });
...
export const deleteTask = catchAsync(async (req, res, next) => {
    ...
    //   req.params.id,
    // ]);
    const result = await pool.query(`DELETE FROM task_base WHERE taskId = ? AND createdOn = ?`, [req.params.id, req.body.createdOn]);
...
export const deleteOne = catchAsync(async (req, res, next) => {
    ...
    //   req.params.id,
    // ]);
    const result = await pool.query(`DELETE FROM task_schedule WHERE relatedTaskId = ? AND scheduledId = ?`, [req.params.id, req.body.createdOn]);
    if (result.affectedRows === 0)
        return next(new AppError("Task not found", 404));
    res
        .status(200)
...
export const updateTask = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const [existingLog] = await connection.query(`SELECT logId FROM task_log WHERE relatedTaskId = ? AND relatedUserId = ? FOR UPDATE`, [req.body.taskId, req.body.relatedUserId]);
        let result;
        // completing the task
        ...
        if (!req.body.logId &&
            ...
            [result] = await connection.query(`INSERT INTO task_log (logId, relatedTaskId, relatedUserId, completedOn) VALUES(?,?,?,?)`, [logId, req.body.taskId, req.body.relatedUserId, completedOn]);
            ...
            [result] = await connection.query(`DELETE FROM task_log WHERE logId = ? AND relatedUserId = ?`, [req.body.logId, req.body.relatedUserId]);
            if (result.affectedRows === 0) {
        ...
        if (req.body.logId) {
            ...
            switch (category) {
                case "health":
                    if (req.user.hp > 0)
                        userProfile.decreaseHP();
                    break;
                case "wealth":
                    if (req.user.wp > 0)
                        userProfile.decreaseWP();
                    break;
                case "knowledge":
                    if (req.user.kp > 0)
                        userProfile.decreaseKP();
                    break;
                default:
                    break;
            }
        ...
        const [updatedUserStat] = await connection.query(`UPDATE user_base 
...
export const addTask = catchAsync(async (req, res, next) => {
    ...
    const [user] = await pool.query(`SELECT * FROM user_base WHERE userId = ?`, [relatedUserId]);
    const taskId = uuid();
    await pool.query(`INSERT INTO task_base (taskId, taskName, taskCategory, taskRepeatsOn, relatedUserId) VALUES(?,?,?,?,?)`, [
        taskId,
        taskName,
        taskCategory,
    ...
    const [todayTask] = await pool.query(`SELECT * FROM task_base WHERE taskId = ?`, [taskId]);
    await scheduleTasks(taskId, relatedUserId, taskRepeatsOn || [], todayTask[0].createdOn);
